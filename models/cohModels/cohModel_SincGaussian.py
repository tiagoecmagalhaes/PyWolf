...
# NumPy
from numpy import int32, double, float32, float, zeros, sin
...
def cohModelFunc(user_interface,context,queue,W_main,N,parameters,parallel,debug):
    ...
    user_interface.update_outputText("Starting Sine Gaussian-Schell model function...")
                    ...
                    for i2 in range(0,N):
                        for j2 in range(0,N):
                            ...
                            if rl == 0.0:
                                ...
                                W_main.real[i1,j1,i2,j2]=W_main.real[i1,j1,i2,j2]*exp(-rl/sigma)*sin(K1*rl)/(K1*rl)
