...
# Pre-requisites
#===============================================================================
cohModel_name = "Lasjunen-Saastamoinen"

cohModel_parameters = ["x0","y0","wc","w0"]
#===============================================================================
#///////////////////////////////////////////////////////////////////////////////
#===============================================================================
...
def cohModelFunc(user_interface,context,queue,W_main,N,parameters,parallel,debug):
    ...
    user_interface.update_outputText("Starting Lasjunen-Saastamoinen model function...")
        ...
        if parallel:
            ...
            CL_pcohNUG=Program(context,"""
                                       ...
                                       const double R1)
                {
                    ...
                    double A=exp(-((x1*x1+y1*y1)+(x22*x22+y22*y22))/(sigma_S));

                    double data_const=(double) data[col + N*row ];


                    double final=data_const*A*arg_miu;
            ...
            for i1 in range(0,N):
                ...
                for j1 in range(0,N):
                    ...
                    CL_pcohNUG.increase(queue,result.shape,None,result_gpu_memory,data_gpu_memory,
                                       ...
                                       double(x1_n),double(y1_n),
                                       double(X0_x),double(X0_y),double(sigma),double(sigma_S),
                                       double(R1))
                ...
                for j1 in range(0,N):
                    ...
                    for i2 in range(0,N):
                        for j2 in range(0,N):
                            ...
                            A=exp(-((x1*x1)+(x2*x2)+(y1*y1)+(y2*y2))/(sigma_S))
                            ...
                            W_main.real[i1,j1,i2,j2]=W_main.real[i1,j1,i2,j2]*miu*A1*A2
