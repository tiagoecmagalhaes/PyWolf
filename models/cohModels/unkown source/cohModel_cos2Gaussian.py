...
cohModel_name = "Cos2 Gaussian Schell-model"
...
def cohModelFunc(user_interface,context,queue,W_main,N,parameters,parallel,debug):
    ...
    user_interface.update_outputText("Starting Cosine squared Gaussian-Schell model function...")
        ...
        if parallel:
            ...
            CL_pcohGS=Program(context,"""
                                       ...
                                       const double K1)
                {
                    ...
                    double COS2  = cos(K1*rl)*cos(K1*rl);
                    double final = data_const*exp2*COS2;
                    ...
                    for i2 in range(0,N):
                        for j2 in range(0,N):
                            ...
                            if rl == 0.0:
                                ...
                                W_main.real[i1,j1,i2,j2]=W_main.real[i1,j1,i2,j2]*exp(-rl/sigma)*cos(K1*rl)**2
