...
def func_startSim(ui,all_parameters):
    ...
    chosen_specDensityModelName =  all_parameters[3][7][0]
    chosen_specDensityModelFunc =  all_parameters[3][7][1]

    # Spectral density model parameters
    specDensityPars =  all_parameters[3][8]
    #___________________________________________________________________________
    ...
    if start_prop==1:
        ...
        try:
            ...
            for numPlane in range(0,numPlanes):
                ...
                a_dev = thr.to_device(ui.CSDM_prop.matrix[0,0])
                ...
                for i1 in range(0,N):
                    ...
                    for j1 in range(0,N):
                        if count_nonzero(ui.CSDM_prop.matrix[i1,j1].real)==0:
                            ...
                            ui.CSDM_prop.matrix[i1,j1] = func_fft2d(ui,ui.CSDM_prop.matrix[i1,j1],False,FFTpad_list,api,thr,FFT,a_dev)
                ...
                for i1 in range(0,N):
                    ...
                    for j1 in range(0,N):
                        if count_nonzero(ui.CSDM_prop.matrix[i1,j1])==0:
                            ...
                            ui.CSDM_prop.matrix[i1,j1]=func_fft2d(ui,ui.CSDM_prop.matrix[i1,j1],True,FFTpad_list,api,thr,FFT,a_dev)
                ui.update_outputTextSameLine("\r"+str(round(100.0,1))+"% concluded")
                ui.update_outputText("2D Fourier transforms completed!")
                ...
                #---------------------------------------------------------------
                # Q functions and lens function
                #---------------------------------------------------------------

                if lens_list[numPlane]:

                    lens_parameters=[]
                    ui.update_outputText("Multiplying by Q functions (with lens)...")

                    C1 = -Cfrequency/(2*float(focalLen_list[numPlane])*c)+Cfrequency/(2*distances_list[numPlane]*c)
                    prop_parameters = [C1]
                ...
                if lens_list[numPlane]:
                    lens_parameters=[]
                    ui.update_outputText("Multiplying by Lens transmission function...")

                    C1 = -Cfrequency/(2*float(focalLen_list[numPlane])*c)
                    lens_parameters = [C1]

                    ui.CSDM_prop.matrix = func_qfunction(ui,context,queue,ui.CSDM_prop.matrix,N,dx,lens_parameters,sim_usePyOpenCL,debug)
                    ui.update_outputText("Task Completed!")
                """
                #_______________________________________________________________
