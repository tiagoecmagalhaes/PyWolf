...
class Canvas_sourceSpecPreview:

    def __init__(self,ui,N):

        # matplotlib area style
        ui.scrollArea_matplot.setStyleSheet("")
    ...
    def build_fig(self):
        ...
        parameters = []
        N = self.N
        Npar = len(self.ui.specModel_lineEditParameters)
        for i in range(0,Npar):
            parameters.append(float(self.ui.specModel_lineEditParameters[i].text()))


        ds = float(self.ui.lineEdit_sourceRes.text())
        numPlanes = int(self.ui.spinBox_numPlanes.text())
        if numPlanes>1:
            ds = self.ui.dx_list[numPlanes-2]


        result = self.ui.specModelsFunc[self.ui.comboBox_specType.currentIndex()](self.ui,self.N,parameters,ds,None)
